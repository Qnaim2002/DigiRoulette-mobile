...
// ✅ "Found Evolution Chip" pool — resolves which tier of chip is found. Omega is the rarest,
// matching Ultra Evolve Chip's original rarity tier — the other weights are unchanged.
export const EVOCHIP_DROP_POOL = [
  {type: "evoChipBasic", label: "🧬 Evolution Chip",       weight: 38},
  {type: "evoChipSuper", label: "🧬 Super Evolve Chip", weight: 28},
  {type: "evoChipMega",  label: "🧬 Mega Evolve Chip",  weight: 19},
  {type: "evoChipUltra", label: "🧬 Ultra Evolve Chip", weight: 10},
  {type: "evoChipOmega", label: "🌟 Omega Evolve Chip", weight: 5}
];
...
const EVO_CHIP_DEFS = [
  ...
  {key: "evoChipSuper", label: "Super Evolve Chip", evoTier: "Adult"},
  {key: "evoChipMega",  label: "Mega Evolve Chip",  evoTier: "Perfect"},
  {key: "evoChipUltra", label: "Ultra Evolve Chip", evoTier: "Ultimate"},
  {key: "evoChipOmega", label: "Omega Evolve Chip", evoTier: "Ultra"},
];
...
// ✅ CSV CACHING: identical scheme to Shop mode's (DigimonRoulette.jsx) — deliberately using
// the SAME cache key prefix, since both modes fetch the exact same 7 Google Sheet URLs. That
// means switching modes (or a "Continue" into either) can reuse whichever mode fetched the
// data most recently, instead of every mode maintaining its own redundant hour-long cache.
...
// the origin bucket — callers additionally check the destination level to split Ultra→Ultra
// from the new Ultra→Ultra+ capstone bucket (see evoTier logic at each call site).
...
// Shop mode, same as music/SFX/reduced-motion settings.
...
export function useDigimonGame(autoResume = false) {
  ...
  // ✅ PERF: one shared CSV-parse worker per hook instance — see utils/csvWorker.js.
  ...
  const loadGameData = (forceRefresh = false) => {
    ...
    const fetchAndParse = (url, label) => {
      ...
      // ✅ PERF: parsed off the main thread in a Web Worker — see utils/csvWorker.js and the
      // matching comment in DigimonRoulette.jsx (Shop mode).
    ...
    .then(([startersData, wildData, eggsData, legendaryData, villainsData, evolutionsData, bossesData]) => {
      ...
      setStarters(startersData.map((d) => {
        // ✅ FIX: preview now applies the same ±15% species variance getStatVariance() that
        // chooseStarter() -> findDigimonInSheetData() applies for real, so the number shown
        // here always matches what the player actually gets.
        const variance = getStatVariance(d.name);
        return {
          dapiName: d.name,
          displayLabel: d.name,
  ...
  useEffect(() => {
    if (!["world_wheel","combat","sub_wheel","party_setup","victory","game_over"].includes(phase)) return;
    // ✅ PERF: see the matching comment in DigimonRoulette.jsx (Shop mode) — same debounce,
    // same immediate-flush exception for run-ending checkpoints.
    if (persistTimeoutRef.current) {clearTimeout(persistTimeoutRef.current); persistTimeoutRef.current = null;}
  ...
  }, [phase, party, reserve, inventory, worldSpinCount, villainWaveStage, score, pendingBattleInit]);
  ...
  const chooseStarter = async (starterObj) => {
    ...
    try {
      ...
      setWorldSpinCount(0); setScore(0); setScoreBreakdown(getDefaultScoreBreakdown());
      worldSpinCountRef.current = 0;
  ...
  return {
    party, reserve, inventory, phase, loadingMsg, loadError, retryDataLoad: () => loadGameData(true), log, announcement, setAnnouncement, starters,
